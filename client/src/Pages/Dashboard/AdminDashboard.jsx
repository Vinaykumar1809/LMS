...
import {deleteLibrary,getAllLibraries} from '../../Redux/Slices/LibrarySlice'
import { getPaymentRecord } from "../../Redux/Slices/StripeSlice";
...
function AdminDashboard() {
    ...
    const { allPayments, monthlySalesRecord } = useSelector((state) => state.stripe);
    ...
    const myLibrary = useSelector((state)=>state?.library?.libraryData);
    ...
    async function onCourseDelete(id) {
     ...
     async function onLibraryDelete(id) {
        if(window.confirm("Are you sure you want to delete this collection ? ")) {
            const res = await dispatch(deleteLibrary(id));
            if(res?.payload?.success) {
            ...
            <div className="min-h-[90vh] pt-5 flex flex-col flex-wrap gap-10 text-white">
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            <div className="flex items-center justify-between p-5 gap-5 rounded-md shadow-md">
                                <div className="flex flex-col items-center">
                                   ...
                                   <h3 className="text-4xl font-bold">{(allPayments && allPayments.length) || 0}</h3>
                                </div>
                ...
                <div className="mx-[10%] w-[80%] self-center flex flex-col items-center justify-center gap-10 mb-10">
                    ...
                    <table className="table overflow-x-scroll">
                            ...
                            <tbody>
                                {myLibrary?.map((library, idx) => {
                                    return (
                                        <tr key={library._id}>
                                            <td>{idx+1}</td>
                                            <td>
                                                <textarea readOnly value={library?.title} className="w-40 h-auto bg-transparent resize-none"></textarea>
                                            </td>
                                            <td>
                                                {library?.category}
                                            </td>
                                            ...
                                            <td className="flex items-center gap-4">
                                                <button
                                                    className="bg-green-500 hover:bg-green-600 transition-all ease-in-out duration-300 text-xl py-2 px-4 rounded-md font-bold"
                                                    onClick={() => navigate("/library/displaybooks", {state: {...library}})}
                                                >
                                                    <BsCollectionPlayFill />
                                                </button>
                                                <button
                                                    className="bg-red-500 hover:bg-red-600 transition-all ease-in-out duration-300 text-xl py-2 px-4 rounded-md font-bold"
                                                    onClick={() => onLibraryDelete(library?._id)}
                                                >
                                                    <BsTrash />
                                                </button>
                                            </td>
